...
import com.aiinterview.cms.service.AdminMetadataService;
...
@RestController
@RequestMapping(RequestMappingConstant.API_VERSION_PREFIX + "/admin")
public class AdminMetadataController {
    ...
    public ResponseEntity<CommonResponse<MetadataResponse>> metadata() {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.getMetadata()));
    }

    @GetMapping("/skills")
    public ResponseEntity<CommonResponse<List<SkillResponse>>> getSkills() {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.getSkills()));
    }

    @PostMapping("/skills")
    public ResponseEntity<CommonResponse<SkillResponse>> createSkill(@Valid @RequestBody SkillRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(CommonResponseFactory.success(adminMetadataService.createSkill(request), "Created"));
    }
    ...
    public ResponseEntity<CommonResponse<SkillResponse>> updateSkill(@PathVariable Integer id, @Valid @RequestBody SkillRequest request) {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.updateSkill(id, request), "Updated"));
    }
    ...
    public ResponseEntity<CommonResponse<TemplateResponse>> updateTemplate(@PathVariable Integer id, @Valid @RequestBody TemplateRequest request) {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.updateTemplate(id, request), "Updated"));
    }

    @DeleteMapping("/templates/{id}")
    public ResponseEntity<CommonResponse<Void>> deleteTemplate(@PathVariable Integer id) {
        adminMetadataService.deleteTemplate(id);
        return ResponseEntity.ok(CommonResponseFactory.success(null, "Deleted"));
    }

    @GetMapping("/questions")
    public ResponseEntity<CommonResponse<List<QuestionResponse>>> getQuestions() {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.getQuestions()));
    }

    @PostMapping("/questions")
    public ResponseEntity<CommonResponse<QuestionResponse>> createQuestion(@Valid @RequestBody QuestionRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(CommonResponseFactory.success(adminMetadataService.createQuestion(request), "Created"));
    }
    ...
    public ResponseEntity<CommonResponse<QuestionResponse>> updateQuestion(@PathVariable UUID id, @Valid @RequestBody QuestionRequest request) {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.updateQuestion(id, request), "Updated"));
    }

    @DeleteMapping("/questions/{id}")
    public ResponseEntity<CommonResponse<Void>> deleteQuestion(@PathVariable UUID id) {
        adminMetadataService.deleteQuestion(id);
        return ResponseEntity.ok(CommonResponseFactory.success(null, "Deleted"));
    }

    @GetMapping("/users")
    public ResponseEntity<CommonResponse<List<UserResponse>>> getUsers() {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.getUsers()));
    }

    @PostMapping("/users")
    public ResponseEntity<CommonResponse<UserResponse>> createUser(@Valid @RequestBody UserRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(CommonResponseFactory.success(adminMetadataService.createUser(request), "Created"));
    }

    @PutMapping("/users/{id}")
    public ResponseEntity<CommonResponse<UserResponse>> updateUser(@PathVariable UUID id, @Valid @RequestBody UserRequest request) {
        return ResponseEntity.ok(CommonResponseFactory.success(adminMetadataService.updateUser(id, request), "Updated"));
    }

    @DeleteMapping("/users/{id}")
    public ResponseEntity<CommonResponse<Void>> deleteUser(@PathVariable UUID id) {
        adminMetadataService.deleteUser(id);
        return ResponseEntity.ok(CommonResponseFactory.success(null, "Deleted"));
    }
}
