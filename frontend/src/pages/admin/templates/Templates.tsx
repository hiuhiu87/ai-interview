...
export default function Templates() {
  ...
  const load = async () => {
    try {
      const metadata = await adminMetadataService.getMetadata();
  ...
  const handleTagSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (tagForm.id) {
        await adminMetadataService.updateTag(tagForm.id, payload);
        toast.success("Tag updated");
      } else {
        await adminMetadataService.createTag(payload);
  ...
  const handleTemplateSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (templateForm.id) {
        await adminMetadataService.updateTemplate(templateForm.id, payload);
        toast.success("Template updated");
      } else {
        await adminMetadataService.createTemplate(payload);
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-slate-950">Templates & Tags</h1>
          <p className="mt-1 text-sm text-slate-500">Manage question templates and tags.</p>
        </div>
      ...
      <div className="grid gap-6 2xl:grid-cols-2">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-950">Tag management</h2>
              <p className="mt-1 text-sm text-slate-500">Use tags to classify questions and templates.</p>
            </div>
            <Button
              onClick={() => {
                setTagForm(defaultTagForm);
          ...
          <div className="mt-6 grid gap-3">
            {tags.map((tag) => (
              <div key={tag.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex shrink-0 gap-1.5">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          await adminMetadataService.deleteTag(tag.id);
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-lg font-semibold text-slate-950">Question templates</h2>
              <p className="mt-1 text-sm text-slate-500">Templates for standardized question sets.</p>
            </div>
            <Button
              onClick={() => {
                setTemplateForm(defaultTemplateForm);
              ...
              <div key={template.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 text-xs uppercase tracking-[0.2em] text-muted-foreground">
                      {template.code}
                      {template.skillName ? ` · ${template.skillName}` : ""}
                      {template.defaultDifficulty ? ` · ${template.defaultDifficulty}` : ""}
                    </div>
                  ...
                  <div className="flex shrink-0 gap-1.5">
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          await adminMetadataService.deleteTemplate(template.id);
          ...
          <form onSubmit={handleTagSubmit} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="space-y-2">
                ...
                <Input
                  value={tagForm.code}
                  onChange={(event) => setTagForm((prev) => ({ ...prev, code: event.target.value.toUpperCase() }))}
                  placeholder="REACT"
                  required
                />
              </label>
              <label className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => setTagForm((prev) => ({ ...prev, category: event.target.value }))}
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={closeTagModal}>
                Cancel
              </Button>
          ...
          <form onSubmit={handleTemplateSubmit} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => setTemplateForm((prev) => ({ ...prev, code: event.target.value.toUpperCase() }))}
                  placeholder="FE_SCREEN"
                  required
                />
              </label>
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-2">
                ...
                <Select
                  value={templateForm.defaultDifficulty || "none"}
                  onValueChange={(value) =>
                    setTemplateForm((prev) => ({
                      ...prev,
                      defaultDifficulty: value === "none" ? "" : value,
                    }))
                  }
                >
              ...
              <label className="space-y-2">
                ...
                <Select
                  value={templateForm.skillId || "none"}
                  onValueChange={(value) =>
                    setTemplateForm((prev) => ({
                      ...prev,
                      skillId: value === "none" ? "" : value,
                    }))
                  }
                >
                  <SelectTrigger className="h-10 w-full">
              ...
              <div className="grid gap-2 md:grid-cols-2">
                {tags.map((tag) => (
                  <label key={tag.id} className="flex items-center gap-3 rounded-2xl border border-border px-3 py-2 text-sm">
                    <input
                      type="checkbox"
                      checked={templateForm.tagIds.includes(tag.id)}
                      onChange={() => toggleTemplateTag(tag.id)}
                    />
                    <span>{tag.name}</span>
                  </label>
                ))}
