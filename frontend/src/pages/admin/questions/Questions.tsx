...
export default function Questions() {
  ...
  const load = async () => {
    try {
      const [metadata, questionItems] = await Promise.all([
        adminMetadataService.getMetadata(),
        adminMetadataService.getQuestions(),
      ]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (form.id) {
        await adminMetadataService.updateQuestion(form.id, payload);
        toast.success("Question updated");
      } else {
        await adminMetadataService.createQuestion(payload);
    ...
    <section className="space-y-6">
      <div className="flex flex-col gap-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-2xl font-semibold text-slate-950">Question Metadata</h1>
          <p className="mt-1 text-sm text-slate-500">Assign difficulty, skill, tag, and template to each question.</p>
        </div>
      ...
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Source data for question sets.</p>
        </div>
              ...
              <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 text-xs uppercase tracking-[0.18em] text-muted-foreground">
                    {question.difficulty || "NO_DIFFICULTY"}
                    {question.skillName ? ` · ${question.skillName}` : ""}
                    {question.templateName ? ` · ${question.templateName}` : ""}
                  </div>
                  <p className="mt-3 text-sm text-muted-foreground">{question.expectedAnswer || "-"}</p>
                ...
                <div className="flex shrink-0 gap-1.5">
                  ...
                  <Button
                    ...
                    onClick={async () => {
                      try {
                        await adminMetadataService.deleteQuestion(question.id);
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <label className="block space-y-2">
              ...
              <textarea
                ...
                placeholder="Explain React reconciliation."
                required
              />
            </label>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <label className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm((prev) => ({ ...prev, difficulty: value === "none" ? "" : value }))}
                >
              ...
              <label className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(value) => setForm((prev) => ({ ...prev, skillId: value === "none" ? "" : value }))}
                >
                  <SelectTrigger className="h-10 w-full">
